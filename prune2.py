...
def parse_args():
    ...
    # Model
    parser.add_argument('--model', default='deit_tiny_distilled_patch16_224.fb_in1k', type=str, help='model name')
    ...
    parser.add_argument('--taylor-batchs', default=500, type=int, help='number of batchs for taylor criterion')
    ...
    # Pruning
    parser.add_argument('--pruning-ratio', default=0.3, type=float, help='prune ratio')
    parser.add_argument('--head-pruning-ratio', default=0.3, type=float, help='head pruning ratio')
    parser.add_argument('--head-dim-pruning-ratio', default=0.1, type=float, help='head dim pruning ratio')
    parser.add_argument('--pruning-type', default='WFRC', type=str, help='pruning type', choices=['random', 'taylor', 'l2', 'l1', 'hessian','WFRC'])
    parser.add_argument('--test-accuracy', default=True, action='store_true', help='test accuracy')
    parser.add_argument('--global-pruning', default=True, action='store_true', help='global pruning')
    parser.add_argument('--save-as', default="output/pruned/deit_0.6G.pth", type=str, help='save the pruned model')
    ...
    parser.add_argument('--cache_dir',default="/lustre/hdd/LAS/jannesar-lab/msamani/.cache",type=str, help="cache directory for datasets")
    args = parser.parse_args()
    return args



def prepare_imagenet_hf(train_batch_size=64, val_batch_size=128, num_workers=4, use_imagenet_mean_std=True, interpolation='bicubic', val_resize=256, cache_dir=None):
    """
    Loads ImageNet using Hugging Face datasets. 
    'cache_dir' replaces 'imagenet_root' to specify where HF should store/find the dataset.
    """
    interpolation = getattr(T.InterpolationMode, interpolation.upper())

    print('Parsing Hugging Face dataset...')
    # Load the dataset. Note: You must have accepted the terms on the HF website 
    # and be logged in via `huggingface-cli login` for imagenet-1k.
    dataset = load_dataset("imagenet-1k", cache_dir=cache_dir)

    # Initialize your existing pbench transform
    base_transform = pbench.data.presets.ClassificationPresetEval(
    ...
    # Hugging Face datasets require a transform function that processes a batch of examples
    def hf_transform(examples):
        # We must convert to RGB because ImageNet contains some grayscale/CMYK images
        # which will crash standard torchvision transforms expecting 3 channels.
    ...
    train_dst = dataset["train"].with_transform(hf_transform)
    val_dst = dataset["validation"].with_transform(hf_transform)

    # Custom collate function to strip the HF dictionary format and return standard (image, label) tuples
...
def main():
    ...
    train_loader, val_loader = prepare_imagenet_hf(cache_dir=args.cache_dir, train_batch_size=args.train_batch_size, val_batch_size=args.val_batch_size, use_imagenet_mean_std= not args.no_imagenet_mean_std, val_resize=args.val_resize, interpolation=args.interpolation)
    ...
    elif args.pruning_type == "WFRC":
        imp = tp.importance.GroupWandaRicciImportance(
            sparsity_threshold_mlp=0.03, 
            sparsity_threshold_attn=0.8
        )
    ...
    if isinstance(imp, tp.importance.GroupWandaRicciImportance):
        # The model is fully loaded and on the correct device here
        imp.prepare_wanda(model, train_loader, device, num_batches=args.taylor_batchs)
